...
def test_target_id_rendered_as_chinese_name() -> None:
    ...
    entry = TimelineEntry(
        turn_id="t1", actor_id="player", kind=TimelineKind.SPEAK,
        speak="你是谁", target_id="baizhantang",
    )
...
def test_speak_with_narration_appended() -> None:
    ...
    entry = TimelineEntry(
        turn_id="t1", actor_id="baizhantang", kind=TimelineKind.SPEAK,
        speak="在下白展堂", narration="拱手行礼",
    )
    rendered = render_timeline_for_messages([entry], npc_name_lookup=_NAMES)
...
def test_pure_act_entry_skipped() -> None:
    ...
    entry = TimelineEntry(
        turn_id="t1", actor_id="baizhantang",
...
def test_narration_only_no_speak() -> None:
    ...
    entry = TimelineEntry(
        turn_id="t1", actor_id="baizhantang", narration="转身离去",
    )
    rendered = render_timeline_for_messages([entry], npc_name_lookup=_NAMES)
...
def test_scene_renders_narration_when_no_speak() -> None:
    ...
    entry = TimelineEntry(
        turn_id="t1", kind=TimelineKind.SCENE, speak=None,
